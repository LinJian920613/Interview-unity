...
public class LTDescr
{
    ...
    public Transform trans
    {
        ...
        set
        {
            if(_trans != value)
            {
                ...
                if(_trans != null)
                {
                    _allUIText = _trans.GetComponentsInChildren<UnityEngine.UI.Text>(true);
                    _allGraphic = _trans.GetComponentsInChildren<UnityEngine.UI.MaskableGraphic>(true);
                }
                else
                {
                    _allUIText = null;
    ...
    UnityEngine.UI.Text[] _allUIText = null;
    UnityEngine.UI.MaskableGraphic[] _allGraphic = null;
    #endregion
	...
	public LTDescr setCanvasAlpha(){
		...
		this.easeInternal = ()=>{
			...
			if(this.useRecursion){
                ...
                alphaAllGraphicChildren(this, val);
                textAlphaAllChildren(this, val);
                #endregion
            }
        };
		return this;
...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
    ...
    #region JJ_REV    
    private static void alphaAllGraphicChildren(LTDescr descr, float val)
    {
        if(descr._allGraphic != null && descr._allGraphic.Length > 0)
        {
            int num = descr._allGraphic.Length;

            for(int i = 0; i < num; i++)
            {
                UnityEngine.UI.MaskableGraphic uiImage = descr._allGraphic[i];
                if(uiImage != null)
                {
    ...
    #region JJ_REV    
    private static void textAlphaAllChildren(LTDescr descr, float val, bool useRecursion = true)
    {
        if(useRecursion && descr._allUIText != null && descr._allUIText.Length > 0)
        {
            int num = descr._allUIText.Length;

            for(int i = 0; i < num; i++)
            {
                UnityEngine.UI.Text uiText = descr._allUIText[i];
                if(uiText != null)
                {
	...
	*/
	...
	private Vector3 tweenOnCurve(){
        #region JJ_REV
        var eval = this._optional.animationCurve.Evaluate(ratioPassed);
        return new Vector3(this.from.x + (this.diff.x) * eval,
            this.from.y + (this.diff.y) * eval,
            this.from.z + (this.diff.z) * eval);
        #endregion
