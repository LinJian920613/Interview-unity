namespace DentedPixel.LTEditor
{
	...
	public class LeanTweenVisual : MonoBehaviour
	{
		...
		private void buildTween(LeanTweenItem item, float delayAdd, bool generateCode)
		{
			...
			//item.action = (TweenAction)System.Enum.Parse(typeof(TweenAction), item.actionStr, true);
			...
			//Debug.Log("item:"+item.action+" itemStr:"+item.actionStr);
			...
			else if (item.action == TweenAction.MOVE_CURVED)
			{
				...
				Vector3[] path_vec3 = item.bezierPath ? item.bezierPath.vec3 : null;
				tween = code ? append("move", path_vec3, d) : LeanTween.move(item.gameObject, path_vec3, d);
				#endregion
			...
			else if (item.action == TweenAction.MOVE_CURVED_LOCAL)
			{
				...
				Vector3[] path_vec3 = item.bezierPath ? item.bezierPath.vec3 : null;
				tween = code ? append("moveLocal", path_vec3, d) : LeanTween.moveLocal(item.gameObject, path_vec3, d);
				#endregion
				if (item.orientToPath)
				{
					if (code)
			...
			else if (item.action == TweenAction.MOVE_SPLINE)
			{
				...
				Vector3[] path_vec3 = item.splinePath ? item.splinePath.splineVector() : null;
				tween = code ? append("moveSpline", path_vec3, d) : LeanTween.moveSpline(item.gameObject, path_vec3, d);
				#endregion
				if (item.orientToPath)
				{
					if (code)
			...
			{
				tween = code ? appendRect("move", item.to, d) : LeanTween.move(item.rectTransform, item.to, d);
			}
			else if (item.action == TweenAction.CANVAS_SCALE)
			{
				tween = code ? appendRect("scale", item.to, d) : LeanTween.scale(item.rectTransform, item.to, d);
			}
			else if (item.action == TweenAction.CANVAS_SIZEDELTA)
			{
				Vector2 to = new Vector2(item.to.x, item.to.y);
				tween = code ? appendRect("size", to, d) : LeanTween.size(item.rectTransform, to, d);
			}
			else if (item.action == TweenAction.CANVAS_ROTATEAROUND)
			{
				...
				{
					tween = LeanTween.rotateAround(item.rectTransform, item.axis, item.to.x, d);
				}
			}
			else if (item.action == TweenAction.CANVAS_ROTATEAROUND_LOCAL)
			{
				...
				{
					tween = LeanTween.rotateAroundLocal(item.rectTransform, item.axis, item.to.x, d);
				}
			}
			else if (item.action == TweenAction.CANVAS_ALPHA)
			{
				tween = code ? appendRect("alpha", item.to.x, d) : LeanTween.alpha(item.rectTransform, item.to.x, d);
			}
			else if (item.action == TweenAction.CANVAS_COLOR)
			{
				tween = code ? appendRect("color", item.colorTo, d) : LeanTween.color(item.rectTransform, item.colorTo, d);
			}
			else if (item.action == TweenAction.TEXT_ALPHA)
			{
				tween = code ? appendRect("textAlpha", item.to.x, d) : LeanTween.textAlpha(item.rectTransform, item.to.x, d);
			}
			else if (item.action == TweenAction.TEXT_COLOR)
			{
				tween = code ? appendRect("textColor", item.colorTo, d) : LeanTween.textColor(item.rectTransform, item.colorTo, d);
			}
			else if (item.action == TweenAction.CANVAS_PLAYSPRITE)
			{
				...
				{
					tween = LeanTween.play(item.rectTransform, item.sprites).setFrameRate(item.frameRate);
				}
			}
			else if (item.action == TweenAction.CANVAS_MOVE_X)
			{
				tween = code ? appendRect("moveX", item.to.x, d) : LeanTween.moveX(item.rectTransform, item.to.x, d);
			}
			else if (item.action == TweenAction.CANVAS_MOVE_Y)
			{
				tween = code ? appendRect("moveY", item.to.x, d) : LeanTween.moveY(item.rectTransform, item.to.x, d);
			}
			else if (item.action == TweenAction.CANVAS_MOVE_Z)
			{
				tween = code ? appendRect("moveZ", item.to.x, d) : LeanTween.moveZ(item.rectTransform, item.to.x, d);
			}
#endif
			...
			#region JJ_DEV
			if (item.useUnscaledTime && tween != null)
			{
	...
	public class LeanTweenItem
	{
		...
		#region JJ_DEV
		public bool useUnscaledTime = false; // defaults to time
		#endregion
