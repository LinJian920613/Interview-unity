			...
			foreach(LeanTweenGroup group in tween.groupList)
			{
				...
				if(group.foldout)
				{
					...
					foreach(LeanTweenItem item in group.itemList)
					{
                        ...
                        //TweenAction a = (TweenAction)System.Enum.Parse(typeof(TweenAction), item.actionStr, true);
                        item.action = a;
...
//								}
							...
							if(isCurve==false){
								bool isVector = a == TweenAction.MOVE || a == TweenAction.MOVE_LOCAL || a == TweenAction.CANVAS_MOVE || a == TweenAction.ROTATE || a == TweenAction.ROTATE_LOCAL ||
									a == TweenAction.SCALE || a == TweenAction.CANVAS_SCALE || a == TweenAction.CANVAS_SIZEDELTA || a == TweenAction.DELAYED_SOUND;
...
//										item.from = EditorGUILayout.Vector3Field("", item.from);
								...
								{
                                    //Debug.Log("a:" + a+" item:"+item.delay+" actionStr:"+item.actionStr);
                            ...
                            #region JJ_DEV
                            // Use Unscaled time or not
                            EditorGUILayout.BeginHorizontal();
                            change(ref item.useUnscaledTime, EditorGUILayout.Toggle("    Use UnscaledTime", item.useUnscaledTime), tween, "toggled use unscaled time");
                            EditorGUILayout.EndHorizontal();
                            #endregion
